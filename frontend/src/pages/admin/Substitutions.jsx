...
const ClassCoverageManagement = () => {
  ...
  const handleConfirmReassignment = async (newTeacherInfo) => {
    if (!selectedSubstitution) return;

    try {
      // This would be the actual reassignment API call
      const response = await api.post('/api/substitutions/override', {
        substitutionId: selectedSubstitution._id,
        newSubstituteId: newTeacherInfo,
        reason: 'Teacher reassigned by administrator'
      });

      setToast({ message: 'Teacher assignment updated successfully.', type: 'success' });
      fetchSubstitutions();

    } catch (err) {
      console.error('Failed to reassign teacher:', err);
      setToast({ message: 'Failed to update teacher assignment.', type: 'error' });
    } finally {
  ...
  return (
    <>
      ...
      <ActionConfirmationModal
            ...
            : selectedSubstitution ? (
                <div className="text-left space-y-3 max-w-md">
                  ...
                  <div className="space-y-2">
                    ...
                    <p><strong>Reason:</strong> <StatusBadge reason={selectedSubstitution.reason} /></p>
                  </div>
                </div>
              ) : ''
        }
        ...
        commentPlaceholder="Enter reason for reassignment..."
      />
